...
function chatHtml() {
  const messages = state.messages.length
    ? state.messages.map((message) => `<div class="chat-message local"><div class="message-meta"><strong>${escapeHtml(message.name)}</strong><span>${escapeHtml(message.time)}</span></div><p>${escapeHtml(message.text)}</p></div>`).join("")
...
function roomHtml() {
  return `<div class="room-app"><header class="topbar"><div class="brand-lockup"><span class="brand-mark"><span></span><span></span><span></span></span><strong>Lonely Cord</strong><span class="brand-divider"></span><span class="room-name">Phòng chính</span></div><div class="room-status"><span class="live-indicator"></span> ĐANG TRỰC TUYẾN</div><div class="topbar-actions"><span class="network-badge">${icon("network", 15)}100%</span><button class="icon-button top-setting" id="open-settings" aria-label="Mở cài đặt">${icon("settings", 19)}</button><div class="top-avatar">${avatarHtml()}</div></div></header>
    <main class="room-main"><div class="room-heading"><div><span class="panel-kicker">KÊNH THOẠI</span><h1>Phòng chính</h1></div><div class="room-heading-actions"><span>${icon("users", 15)} 1 người</span><span class="quality-chip">${state.resolution} · ${state.fps} fps</span></div></div>
      <div class="room-layout"><section class="stage"><div class="stage-bar"><span>CUỘC GỌI NHÓM</span><span class="stage-hint">Một phòng duy nhất · kết nối trong trình duyệt</span></div><div class="participant-grid">${participantTileHtml()}${shareTileHtml()}</div></section>${state.chatOpen ? chatHtml() : ""}</div>
...
function bindRoom() {
  ...
  document.querySelector("#chat-form")?.addEventListener("submit", (event) => {
    ...
    if (!text) return;
    state.messages.push({ name: state.displayName, text, time: new Date().toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) });
    state.draft = "";
    render();
    state.chatOpen = true;
  });
